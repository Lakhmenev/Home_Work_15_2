...
CREATE TABLE IF NOT EXISTS dict_colors
...
( id integer PRIMARY KEY AUTOINCREMENT,
  ...
  FOREIGN KEY (one_color_id) REFERENCES dict_colors(id) ON DELETE SET NULL ON UPDATE CASCADE,
  FOREIGN KEY (two_color_id) REFERENCES dict_colors(id) ON DELETE SET NULL ON UPDATE CASCADE,
...
SET one_color_id = (SELECT dict_colors.id  FROM dict_colors WHERE lower(trim(animals_normal.one_color)) = lower(trim(dict_colors.name_color)))
WHERE animals_normal.one_color_id IS NULL;

--ќбновление кода цвета 2
UPDATE animals_normal
SET two_color_id  = (SELECT dict_colors.id  FROM dict_colors WHERE lower(trim(animals_normal.two_color)) = lower(trim(dict_colors.name_color)))
